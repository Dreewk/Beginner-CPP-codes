#include<stdio.h>
#include<malloc.h>                           //For standard input/output functions #include<stdlib.h>                               //For standard library functions
...
while(dat)
{
scanf("%d",&dat);                           //Getting the data from user if(dat==0)                                       //If data entered is '0' then assume its the end of inputting elements break;
...
flag=0;                                                                //Setting the initial value of 'flag' be '0' while(1)
{
scanf("%d",&dat);
if(dat==0)
break;
...
printf("%d ",stack[top]);        //Printing the DFS result top--;
k=p->adj;
while(k!=NULL)
{
q=k->next;
...
{
printf("1: DFS\n2: BSF\n0: Exit\nEnter your choice: ");
scanf("%d",&ch);             //User enters his choice switch(ch)
{
case 1:
dfs();                                   //For Depth First Traversal break;
case 2:
bfs();                                  //For Breadth First Traversal
break;
case 0:
exit(0);                             //If Zero then exit the program (Omit this line if you don't want that) break;
default:
